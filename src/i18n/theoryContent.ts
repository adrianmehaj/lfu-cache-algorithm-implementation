  ...
  sections: [
    {
      ...
      body:
        'A cache is a fast, capacity-limited store that mirrors a subset of data held in a slower backing store (disk, network, or larger RAM). Each lookup tries the cache first so repeated accesses avoid paying the full slow-path cost. When the cache is full, admitting a new item forces an eviction — a policy decides which resident entry to discard. In plain terms: a small shelf beside a huge warehouse. Only a few boxes fit on the shelf; the eviction rule answers “which box goes back to storage when I need space?”',
    },
    {
      title: 'What is LFU (Least Frequently Used)?',
      body:
        'LFU evicts the key with the lowest access frequency among keys currently in the cache. Successful operations (typically PUT and GET hit) bump that key’s counter. On eviction under capacity pressure, the victim is the minimum-frequency key.',
      list: [
    ...
    },
    {
      ...
      body:
        'LRU looks only at time: evict whoever was touched longest ago. LFU looks at counts: evict whoever was touched the fewest times overall. “One-hit wonders” can linger under LRU; steady hot keys are rewarded by LFU. No single policy wins every workload — recency-heavy traffic suits LRU; stable popularity suits LFU.',
      codeBlock: [
...
Evict → B   (LRU within freq=1)`,
    },
    {
      ...
      body:
        'Big-O describes how work grows with problem size n — here, roughly the number of keys in the cache. O(1) means each GET or PUT does a bounded number of steps (map lookups, a few link updates); cost does not scale linearly with n. O(n) would mean touching every entry, e.g. a full scan to pick a victim.',
      codeBlock: [
    ...
    },
    {
      ...
      body:
        'Replacement policies appear wherever a small fast tier sits in front of a slower one. Production systems often blend recency and frequency rather than pure LFU alone — but the same vocabulary applies.',
      list: [
    ...
    },
    {
      title: 'Key takeaways',
      body:
        'Short recap of the main ideas covered above.',
      list: [
...
const sq: TheoryBundle = {
  ...
  sections: [
    ...
    },
    {
      ...
      body:
        'LRU shikon vetëm kohën: dëbon atë që nuk është prekur më gjatë. LFU shikon numërimin: dëbon atë që është prekur më pak herë në total. “Një herë dhe u largua” mund të mbijetojë më gjatë me LRU; çelësat e qëndrueshëm të nxehtë favorizohen nga LFU. Asnjë politikë nuk fiton çdo ngarkesë — trafiku i orientuar nga “e fundit” përshtatet me LRU; popullariteti i qëndrueshëm me LFU.',
      codeBlock: [
    ...
    },
    {
      title: 'FIFO (First In First Out)',
      body:
        'FIFO dëbon çelësin që është futur më herët në cache — radhë: i pari që hyn, i pari që del kur duhet vend. Nuk përdor frekuencën e akseseve (si LFU) as “koha nga prekja e fundit” si rregull kryesor (si LRU). Është i thjeshtë, i parashikueshëm dhe një bazë standarde mësimore. Në ngarkesa me “çelësa të nxehtë” ose shpërndarje të skewed, LFU o LRU shpesh japin hit rate më të mirë; FIFO mbetet pikë referimi e qartë.',
      list: [
...
Dëbim → B   (LRU brenda freq=1)`,
    },
    {
      ...
      body:
        'Big-O përshkruan si rritet kostoja me madhësinë n të problemit — këtu, afërsisht numri i çelësave në cache. O(1) do të thotë se çdo GET ose PUT kryen një numër të kufizuar hapash (kërkime në mapë, disa përditësime lidhjesh); kostoja nuk shkallëzohet linearisht me n. O(n) do të thotë prekje e çdo hyrjeje, p.sh. skanim i plotë për të zgjedhur viktimën.',
      codeBlock: [
    ...
    },
    {
      ...
      body:
        'Politikat e zëvendësimit shfaqen kudo ku një shtresë e shpejtë dhe e vogël qëndron përpara një të ngadaltë. Në prodhim shpesh përdoren hibride recency + frequency — por fjalori është i njëjti.',
      list: [
    ...
    },
    {
      ...
      body:
        'Përmbledhje e shkurtër e ideve kryesore të trajtuara më sipër.',
      list: [
