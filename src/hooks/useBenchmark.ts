...
function genKey(i: number, type: WorkloadType, keySpace: number): number {
  switch (type) {
    ...
    case 'sequential':
      return i % keySpace;
    case 'temporal':
      return Math.max(0, i - Math.floor(Math.random() * 50)) % keySpace;
  }
}
...
export function useBenchmark() {
  ...
  const run = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      const ops = Array.from({ length: totalOps }, (_, i) => ({
        r: Math.random() < readRatio,
        k: genKey(i, workload, ks),
      }));
